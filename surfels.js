...
import { vec4, flatten, perspective, lookAt, rotateX, rotateY, initFileShaders, vec2 } from "./helperfunctions.js";
"use strict";
let gl;
let program;
//uniform locations
...
function readTextFile(filePath) {
    fetch(filePath)
        ...
        .then(text => {
        createPointCloud(text); //ok, we have our data, so parse it
...
function createPointCloud(input) {
    ...
    if (onPoints) {
        for (let i = 49; i < 10 * numVerts + 49; i += 10) {
...
function makeTexture() {
    ...
    let rowOneFiveValues = [0, 50, 100, 50, 0];
    let rowTwoFourValues = [50, 175, 255, 175, 50];
    let rowThreeValues = [100, 255, 255, 255, 100];
...
//# sourceMappingURL=surfels.js.map